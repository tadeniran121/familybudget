...
import { getCategoryAnnualTotal } from '../utils/calculations.js'
...
function BudgetCell({ value, onChange, currency }) {
  ...
  return (
    <td className="px-2 py-1 text-center w-20">
      <button
        onClick={startEdit}
        className="w-full text-xs rounded px-1.5 py-1 text-right hover:bg-slate-100 transition-colors focus:outline-none focus:ring-1 focus:ring-brand-500 rounded"
        aria-label={`Edit budget: ${value > 0 ? formatCurrency(value, currency) : '—'}`}
      >
...
export default function Budget() {
  ...
  // Net row: income - savings - expenses per month
  const monthNets = months.map(m => {
    let income = 0, expenses = 0
    enabledGroups.forEach(g => {
      g.categories.filter(c => c.enabled).forEach(cat => {
        const v = Number(m.budget?.[cat.id]) || 0
  ...
  const annualNet = monthNets.reduce((s, v) => s + v, 0)
    ...
    <div className="space-y-4">
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="text-left px-4 py-3 font-semibold text-slate-700 sticky left-0 bg-slate-50 min-w-[180px] z-10">
                  Category
                </th>
                {MONTH_SHORT.map(m => (
                ...
                <th className="text-center px-3 py-3 font-semibold text-slate-700 text-xs w-20">
                  Total
                </th>
              </tr>
            </thead>
            <tbody>
              {enabledGroups.map(group => {
                ...
                const groupColor = group.type === 'income' ? 'bg-emerald-50' : group.type === 'savings' ? 'bg-brand-50' : 'bg-slate-50'
                const groupTextColor = group.type === 'income' ? 'text-emerald-800' : group.type === 'savings' ? 'text-brand-800' : 'text-slate-700'
                  ...
                  <>
                    ...
                    <tr key={`${group.id}-header`} className={`${groupColor} border-y border-slate-200`}>
                      ...
                      <td className={`text-center px-3 py-2 text-xs font-semibold ${groupTextColor}`}>
                        {formatShort(
                          enabledCats.reduce((s, c) => s + getCategoryAnnualTotal(months, c.id), 0),
                          currency
                        )}
                      </td>
                    </tr>
              ...
              <tr className="bg-slate-800 text-white border-t-2 border-slate-700">
                <td className="px-4 py-3 sticky left-0 bg-slate-800 z-10 font-bold text-sm">
                  Net Balance
                </td>
                {monthNets.map((net, i) => (
                  <td
                    key={i}
                    className={`text-center px-2 py-3 text-xs font-bold ${net >= 0 ? 'text-emerald-400' : 'text-red-400'}`}
                  >
      ...
      >
        <p className="text-sm text-slate-500 mb-5">
          Copy all budget values from one month to another. This will overwrite the destination month.
        </p>
