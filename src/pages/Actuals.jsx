...
function ActualInput({ value, onChange }) {
  ...
  return (
    <button
      ...
      aria-label={value > 0 ? `Actual: ${value}` : 'Click to enter actual amount'}
    >
      {value > 0
        ? <span className="text-slate-800 font-medium">{formatCurrency(value)}</span>
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="text-left px-4 py-3 font-semibold text-slate-700 min-w-[180px]">Category</th>
            ...
            <tbody>
              {enabledGroups.map(group => {
                ...
                const groupText = group.type === 'income' ? 'text-emerald-800' : group.type === 'savings' ? 'text-brand-800' : 'text-slate-700'
                  ...
                  <>
                    ...
                    {enabledCats.map(cat => {
                      ...
                      return (
                        <tr
                          key={cat.id}
                          className={`border-b border-slate-100 transition-colors ${isOver ? 'bg-red-50' : 'hover:bg-slate-50/50'}`}
                        >
                          <td className="px-4 py-2">
                            <span className="pl-4 text-slate-700 text-xs">{cat.label}</span>
                          </td>
                          <td className="text-right px-4 py-2 text-slate-500 text-xs">
                            {budget > 0 ? formatCurrency(budget, currency) : <span className="text-slate-300">—</span>}
                          </td>
                          <td className="px-4 py-2">
                            <ActualInput
                              value={actual}
                              onChange={v => handleActual(cat.id, v)}
                            />
                          </td>
                          <td className="text-right px-4 py-2">
              ...
              <tr className="bg-slate-800 text-white border-t-2 border-slate-700">
                ...
                <td className="text-right px-4 py-3 text-sm font-bold">
                  {formatCurrency(totalBudgetExpense, currency)}
                </td>
                <td className="text-right px-4 py-3 text-sm font-bold">
